...
class nnUNetLogger(object):
    ...
    def log(self, key, value, epoch: int):
        ...
        if key == 'mean_fg_dice':
            ...
            # determine previous EMA safely
            if len(ema_list) >= 1:
                prev_ema = ema_list[-1]     # safest, never out of bounds
            else:
                prev_ema = value            # initialize EMA with first value
            
            new_ema = prev_ema * 0.9 + 0.1 * value
