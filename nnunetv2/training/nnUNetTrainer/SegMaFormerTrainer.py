...
class SegMaFormerTrainer(nnUNetTrainer):
    ...
    def _build_loss(self):
        """
        Custom Dice-only loss for SegMaFormer + nnU-Net training.
        ...
        if self._do_i_compile():
            try:
                ...
                print(f"⚠️ Warning: torch.compile() failed for Dice loss: {e}")
        ...
        if self.enable_deep_supervision:
            ...
            weights = np.array([1.0, 0.0625, 0.03125, 0.03125, 0.03125], dtype=np.float32)
...
class SegMaFormerTrainer_BTCV_Legacy(SegMaFormerTrainer):
    config_filename = "BCTV_config_legacy.yml"


class SegMaFormerTrainer_BTCV_Fixed(SegMaFormerTrainer):
